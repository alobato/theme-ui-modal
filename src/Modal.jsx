...
const Modal = ({ children, onCancel, closeOnClickOutside = true, shown, lockBodyScroll = true, backdrop = true, immediate = false, backdropColor = 'hsl(0deg 0% 0% / 70%)', sx, variant }) => {

  useLockBodyScroll(shown && lockBodyScroll)
        ...
        <ModalBody>
          {backdrop ? (
            <div ref={parentDiv} onKeyDown={keyHandler} tabIndex={0} aria-modal='true' role='dialog' style={{ position: 'fixed', top: 0, right: 0, bottom: 0, left: 0, overflow: 'hidden', outline: 'none' }}>
              ...
              {backdrop && (
                <animated.div onClick={() => { if (closeOnClickOutside) { onCancel() } }} style={{ backgroundColor: backdropColor, opacity: style.opacity, position: 'fixed', zIndex: 1000, top: 0, right: 0, bottom: 0, left: 0, outline: 'none', tabIndex: -1 }} />
              )}
            </div>
          ) : (
